...
class IssuedBook : public Book {
    ...
    public:
        ...
        void returnBook() {
            this->isIssued = false;
            cout << "\n----------------------------------------------\n";
            cout << "Book ID " << this->bookID << " is returned successfully!" << endl;
            cout << "----------------------------------------------\n\n";
            this->issuedTo = "";
            this->issueDate = "";
        }

        void fineCalcutale(int delayedDays){
            if (!this->isIssued) {
                cout << "Book is not issued, so no fine." << endl;
                return;
            }
            if(delayedDays <= dueDays){
                cout << "No Fine. Returned on time!" << endl;
            } else {
                int extraDays = delayedDays - dueDays;
                int fine = extraDays * finePerDay;
                cout << "\n----------------------------------------------\n";
                cout << "Book ID : " << this->bookID << endl;
                cout << "Issued To : " << this->issuedTo << endl;
                cout << "Delayed Days : " << extraDays << endl;
                cout << "Fine To Pay : Rs." << fine << endl;
                cout << "----------------------------------------------\n\n";
            }
        }
};
    ...
    do {
        ...
        switch(choice) {
            ...
            case 4: {
                ...
                if(!flag) cout << "Book not found in records..." << endl;
                break;
            }
            case 5: {
                int bookID, delayedDays;
                bool flag = false;

                cout << "Enter Book ID to calculate fine: ";
                cin >> bookID;

                cout << "Enter total days after issue: ";
                cin >> delayedDays;

                for(int i = 0; i < index; i++){
                    if(l1[i].getBookId() == bookID){
                        flag = true;
                        l1[i].fineCalcutale(delayedDays);
                        break;
                    }
                }
                if(!flag) cout << "Book not found in records..." << endl;
